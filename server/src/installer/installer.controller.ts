...
export class InstallerController {
        ...
        if (await this.installerService.isInstalled()) {
            setTimeout(async (): Promise<void> => {
                response.status(404).response.send({
                    statusCode: 404, message: 'Cannot GET ' + request.path, error: 'Not Found'
                })
            }, 2000)
            return;
        }
    ...
    async put(@Req() request, @Res() response): Promise<void> {
        ...
        if (!request.body.email || !request.body.password || !request.body.firebase || !request.body.privateKey) {
            response.status(404).send();
            return;
        }

        if (!await this.installerService.install(request.body.email, request.body.password, request.body.firebase, request.body.privateKey)) {
