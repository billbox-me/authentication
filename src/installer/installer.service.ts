...
export class InstallerService {
  ...
  constructor(
    ...
    @InjectModel(UserGroupsModel) private userGroupsModel: typeof UserGroupsModel,
  ...
  async install(email: string, password: string, firebase: string, privateKey: string): Promise<boolean> {
    ...
    try {
       ...
       */
      const passwordsha256: string = await createHmac('sha256', password).digest('hex'); // hash password to sha256
